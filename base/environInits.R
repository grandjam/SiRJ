...
simRspSitType = .2 # 0-1; degree of similarity between response and broadly different situations. If this value is high, the agent will tend to have highly similar responses across all broad situations
simRspSpecSit = .75 # 0-1; degree of similarity between response and unique situations within each broadly different type of situation. If this value is high, the agent will tend to have highly similar responses for specific situations within each broad situation
...
rspTypeExpt <- sapply(1:nrow(rspRandExpt), function(x) t(replicate(numSitType, makeSim(rspRandExpt[x,], simRspSitType))), simplify = F) # Within each matrix, rows share simRspSitType% of features; responses can be perceived more or less similarly, but degree of similarity is constant across exptLvls
...
### Response traces are clustered such that responses to specific situations within a broad situation type share simRspSpecSit% of features for a given expertise level
rspExptLvls <- lapply(rspTypeExpt, function(x) {
  matrix(sapply(1:numSitType, function(y) replicate(numSpecSit, makeSim(x[y,], simRspSpecSit))), nrow = numSpecSit*numSitType, byrow = T)
})
...
rspTypeCxtExpt <- sapply(1:nrow(rspRandCxtExpt), function(x) t(replicate(numSitType, makeSim(rspRandCxtExpt[x,], simRspSitType))), simplify = F) # Within each matrix, rows share simRspSitType% of features; interpretations of goals accomplished by responses to broad situations can be perceived more or less similarly, but degree of similarity is constant across exptLvls

### List of length numExptLvls; each element contains (numSpecSit*numSitType) x (mvLength*numSitCxtMV) matrix representing prototypical interpretation for each specific situation at each expertise level
### Response context traces are clustered such that interpretations of responses to specific situations within a broad situation type share simRspSpecSit% of features for a given expertise level

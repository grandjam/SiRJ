########################################
# Create memory structures for persons #
########################################
createPersonMem <- function(exptLvl, expLvl, mvLength, numSitMv, numRspMv, numSitCxtMv, numRspCxtMv, numSitType, numSpecSit, sitSpecProb, tLen, numExptLvls, sitSpecProto, sitCxtExptLvls, rspExptLvls, rspCxtExptLvls, numSjtItems, sjtSitNdx) {
  # Initialize person parameters
  expStrength = expLvl+4 # Scale up expLvl input values from app (1,2,3,4) to match values used in simulation (5,6,7,8)
  ...
  epMemory[,c(((mvLength*numSitMv)+(mvLength*numSitCxtMv)+1):(((mvLength*numSitMv)+(mvLength*numSitCxtMv))+(mvLength*numRspMv)),(ncol(epMemory)-1))] <- do.call("rbind", sapply(ndxTrace, function(x) {
    epMemRow <- (cumsum(tFreqExp[1:x])[x-1]+1):(cumsum(tFreqExp[1:x])[x])
    exptSamp <- sapply(epMemory$sitCxtLvl[epMemRow], function(z) rspSit(z, S, numExptLvls))
  ...
  # Tabulate situation context, response, and response context levels for easy plotting
  sitCxtLvlCnt = as.data.frame(table(factor(epMemory$sitCxtLvl, levels = 1:5)))
  rspLvlCnt = as.data.frame(table(factor(epMemory$rspLvl, levels = 1:5)))
  rspCxtLvlCnt = as.data.frame(table(factor(epMemory$rspCxtLvl, levels = 1:5)))
  ## Turn any zeros into NAs for plotting
  sitCxtLvlCnt$Freq[sitCxtLvlCnt$Freq == 0] = NA
  rspLvlCnt$Freq[rspLvlCnt$Freq == 0] = NA
  rspCxtLvlCnt$Freq[rspCxtLvlCnt$Freq == 0] = NA
  ...
  return(list("epMemSize" = sum(epMemCnt),
              "sitCxtLvlCnt" = sitCxtLvlCnt,
              "rspLvlCnt" = rspLvlCnt,
              "rspCxtLvlCnt" = rspCxtLvlCnt,
              "epMemCntVec" = epMemCnt,
              "epMemory" = epMemory,
              "semMemory" = semMemory,
              "sjtEval" = sjtEval
  ))

...
rspSit <- function(sitCxtExpt, S, numExptLvls) {
...
if (length(actSemMemTrace) > 1) {
  ...
  while (k < Tmax) {
    ...
    if (!interaction(RSit[,1], RSit[,2]) %in% interaction(SOC$sitType, SOC$sitSpec, drop = T) & actSemMemTraceLvl[RSampled] > Amin) { # Checks whether sampled response is NOT already in SOC and if activation level of sampled response is greater than current minimum activation threshold for SOC inclusion
      ...
      SOC[SOCndx,] <- c(semMemory[RSampled,], actSemMemTraceLvl[RSampled])
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
# (4) Function for answering SJT items #
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
...
###############################################################################
  ...
  ## Number of features that can be probed is set to be limited by WM (phi)
  if (phi < numSitMv) {
...
####################################################################################
# Step 2: Generate response (genRsp) based on interpretation of situation (sitCxt) #
####################################################################################
...
##########################################################################################################
# Step 3: Generate interpretation of each SJT rsp (givRspCxt) given SJT rsp and sitCxt (givRsp & sitCxt) #
##########################################################################################################

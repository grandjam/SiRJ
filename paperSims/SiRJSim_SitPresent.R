...
# library(parallel)
# sfInit(parallel = T, cpus = detectCores()-1, type = "SOCK")
...
source("readItemSitPresent.R") # Loads in algorithm in which item stems are presented & interpreted by SJT respondents
...
save(conds, file = paste("condsSit",id,".RData",sep = ""))
save(condsDat, file = paste("condsDatSit",id,".RData",sep = ""))
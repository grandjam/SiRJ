...
#####################
  ...
  # Evaluate responses
    ...
    while (evalNum < sjtMax) {
      ...
      ###################################################################################################################################################################
      # Step 5: Compute similarity between givRsp for each rsp and genRsp (i.e., evaluate whether actual response option is similar to generated response consequences) #
      ###################################################################################################################################################################
      # Compute similarity using similarity between generated response and given response
      # evalRslt[evalNum+1,,sjtItem] <- sapply(1:nrow(sjtRsp[[sjtItem]]), function(x) {
      #   tSimLvl(sjtRsp[[sjtItem]][x,], genRspSOC$SOC[rspSlct, ((numSitMv*mvLength)+(numSitCxtMv*mvLength)+1):((numSitMv*mvLength)+(numSitCxtMv*mvLength)+(numRspMv*mvLength))])
      # })
      # Compute similarity using similarity between generated response context and given response context
      ...
      ## Check if both ME and LE responses have been made. If so, quit evaluating; if not continue evaluating if there is time/motivation to do so
